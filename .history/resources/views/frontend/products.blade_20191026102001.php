    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                //    alert('success');
                       $.each(data.categories,function(index,valu){
                        if(valu.parent_id==0   ){
                          	$('#catlist').append(
                                     '  <li id="childcat-'+valu.id+'" class=" " ><a href="javascript:void(0)" onclick="get()"> '+ valu.name+'</a></li>'
                ...
                success:function(data){
                    $.each(data.categories,function(index,valu){
                        ...
                        if(_id==valu.parent_id ){
                            output="&nbsp;&nbsp;&nbsp; ->"+valu.name
                            $('#childcat-'+valu.parent_id).append(
                                '<br/><input type="checkbox" name="'+valu.id +'" value="'+valu.id+'" onclick="get2()">'+output
