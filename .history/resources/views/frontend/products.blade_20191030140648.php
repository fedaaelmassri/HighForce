...
@section('js')
                       ...
                       $.each(data.categories,function(index,valu){
                        if(valu.parent_id==0   ){
                          	$('#catlist').append(
                                     ...
                                     '  <li   class=" " ><a   data-toggle="collapse"  data-target="#childcat-'+valu.id+'"   onclick="viewByCat('+valu.id+')"> '+ valu.name+'</a> <ul class="collapse list list-border angle-double-right" id="childcat-'+valu.id+'"></ul> </li> '
                ...
                success:function(data){
                    ...
                    $.each(data.categories,function(index,valu){
                        if(_id==valu.parent_id ){
                            output="&nbsp;&nbsp;&nbsp; "+valu.name
                            $("#childcat-"+id).append(
                                '  <li    class=" " > <a   data-toggle="collapse"  data-target="#childpcat-'+valu.id+'" onclick="viewByCat('+valu.id+')"  >  <i  class="fa fa-angle-right"></i> &nbsp;&nbsp;'+ valu.name+'</a> <ul class="collapse list list-border angle-double-right" id="childpcat-'+valu.id+'"></ul> </li> '
			...
			if((_id2==valu.parent_id && _id2!=_id)   ){x=0;
				_id3=valu.id;
                    ...
                    //  '<br/><input type="checkbox" name="vehicle3" value="'+valu.id+'" checked>'+output
                     '  <li    class=" " > <a   data-toggle="collapse"  onclick="viewByCat('+valu.id+')" data-target="#childppcat-'+valu.id+'"  ><i  class="fa fa-angle-right"></i> &nbsp;&nbsp; '+ valu.name+'</a>  <ul class="collapse list list-border angle-double-right" id="childppcat-'+valu.id+'"></ul> </li> '

                      );
			 }
			if(_id3==valu.parent_id  && x==0){x=1;
 				  $('#childppcat-'+id).append(
            '<li    class=" " > <a   data-toggle="collapse" data-target="#childpppcat-'+valu.id+'" onclick="viewByCat('+valu.id+')" > '+ valu.name+'</a>   </li> '
                         );
			}
 ...
 function viewByCat(id) {
    var url_m='{{URL::to('/productsByCat')}}';
