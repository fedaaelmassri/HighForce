  ...
  <div class="container mt-30 mb-30 p-30">
    <div class="section-content">
      <div class="row">
        ...
        <input type="hidden" id="braid" name="braid">
...
<!-- <input type="checkbox" value="437" id="chk_437" onclick="return getProductList(this.value);"> -->
        ...
        <input type="checkbox" name="{{$brand->name}}-{{$brand->id}}" id="bra-{{$brand->id}}" value="{{$brand->id}}"  onclick="viewByBraCat({{$brand->id}});">
...
@section('js')
                       ...
                       $.each(data.categories,function(index,valu){
                        if(valu.parent_id==0   ){
                          	$('#catlist').append(
                                     ...
                                     '  <li   class=" " ><a   data-toggle="collapse" href="javascript:void(0);" data-target="#childcat-'+valu.id+'"   onclick="viewByCat('+valu.id+')"> '+ valu.name+'</a> <ul class="collapse list angle-double-right" id="childcat-'+valu.id+'"></ul> </li> '
                ...
                success:function(data){
                    ...
                    $.each(data.categories,function(index,valu){
                        if(_id==valu.parent_id ){
                            output="&nbsp;&nbsp;&nbsp; "+valu.name
                            $("#childcat-"+id).append(
                                '  <li    class=" " > <a   data-toggle="collapse" href="javascript:void(0);" data-target="#childpcat-'+valu.id+'" onclick="viewByCat('+valu.id+')"  >  <i  class="fa fa-angle-right"></i> &nbsp;&nbsp;'+ valu.name+'</a> <ul class="collapse list angle-double-right" id="childpcat-'+valu.id+'"></ul> </li> '
			...
			if((_id2==valu.parent_id && _id2!=_id)   ){x=0;
				_id3=valu.id;
                    ...
                    //  '<br/><input type="checkbox" name="vehicle3" value="'+valu.id+'" checked>'+output
                     '  <li    class=" " > <a   data-toggle="collapse" href="javascript:void(0);" onclick="viewByCat('+valu.id+')" data-target="#childppcat-'+valu.id+'"  ><i  class="fa fa-angle-right"></i> &nbsp;&nbsp; '+ valu.name+'</a>  <ul class="collapse list angle-double-right" id="childppcat-'+valu.id+'"></ul> </li> '
 ...
 function viewByBraCat(bid){
    ...
    var cid=$("#catid").val();
     ...
     //document.getElementById("agreed")
    var checkBox=document.getElementById("bra-"+bid).checked;
    var idx = gBrands.indexOf(bid);
      if (checkBox == true && idx == -1){
            gBrands.push(bid);
      } else if (idx != -1) {
          gBrands.splice(gBrands.indexOf(bid), 1);
      }
            ...
            $.ajax({
                ...
                $.each(data.products,function(index,value){
                        $("#productsfilter").append(
                            '<div class="col-sm-6 col-md-4 col-lg-4 mb-30">'+
                            '<div class="product pb-0">'+
                            '<div class="product-thumb">'+
                            '<img alt="" src="'+base_url+'/storage/'+ value.image+'" width="320" heigt="360" class="img-responsive img-fullwidth">'+
                              '</div>'+
                                 '<div class="product-details text-center bg-lighter pt-15 pb-10">'+
                                 '<a href="#"><h5 class="product-title mt-0">'+value.name+'</h5></a>'+
                                 '</div>'+
                                 '</div>'+
                                 '</div>'
                        );
