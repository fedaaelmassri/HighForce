    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
	 ...
	 success:function(data){
		 $.each(data.categories,function(index,valu){
				...
				output="&nbsp; &nbsp; &nbsp; -->"+valu.name
 				  $('#childpcat-'+id).append(
                    //  '<br/><input type="checkbox" name="vehicle3" value="'+valu.id+'" checked>'+output
                     '  <li    class=" " > <a   data-toggle="collapse" href="javascript:void(0);" data-target="#childppcat-'+valu.id+'"  ><i  class="fa fa-angle-right"></i> &nbsp;&nbsp; '+ valu.name+'</a> <ul class="collapse" id="childppcat-'+valu.id+'"></ul> </li> '
            ...
            '  &nbsp; &nbsp  <li    class=" " > <a   data-toggle="collapse" href="javascript:void(0);" data-target="#childpppcat-'+valu.id+'"  > '+ valu.name+'</a>   </li> '
                         );
			}
