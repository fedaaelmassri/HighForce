...
@section('js')
                       ...
                       $.each(data.categories,function(index,valu){
                        if(valu.parent_id==0   ){
                          	$('#catlist').append(
                                     ...
                                     '  <li   class=" " ><a   data-toggle="collapse" href="javascript:void(0);" data-target="#childcat-'+valu.id+'" onclick="get('+valu.id+')"> '+ valu.name+'</a> <ul class="collapse" id="childcat-'+valu.id+'"></ul> </li> '
    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                success:function(data){
                    $('#childcat-'+id).html(' ');
                    $.each(data.categories,function(index,valu){
                        if(id==valu.parent_id ){
                            output="&nbsp;&nbsp;&nbsp; "+valu.name
                            $("#childcat-"+id).append(
                                '<br/>&nbsp;&nbsp;&nbsp; <i  class="fa fa-angle-right"></i><input type="checkbox" id="childpcat-'+valu.id +'" name="'+valu.id +'" value="'+valu.id+'" onclick="get2()">'+output
