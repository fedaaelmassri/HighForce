...
public function editparentCat($id)
{
    ...
    if (!$Category) {
        ...
        ->where('parent_id', $id);
        // $parentid=Categories::where('parent_id', $id)->findOrfail();
        // $Category->delete();
        if($Category->delete()){
        if($parentid){
            $parentCategory= DB::table('Categories')
        ->where('parent_id', $id)
        ->update(['parent_id' => 0]);
            // $parentCategory= Categories::where('parent_id', $id)
            // ->update(['parent_id' => 0]);

           if($parentCategory ){
            return response()->json(['message' => sprintf('The Category: "%s" Edited success !', $Category->name)]);
        }else{
            return response()->json(['message' => sprintf('Error Edited!', $Category->name)]);


        }

    }
    return response()->json(['message' => sprintf('The Category: "%s" Deleeted success !', $parentid)]);
