...
class EcataloguesController extends Controller
{
    ...
    public function store(Request $request){
        ...
        return redirect(route('admin.ecatalogues'))->with('message', sprintf('ECatalogue "%s" add!',$filename));
    ...
    public function delete(Request $req)
    {
        ...
        $ecatalogues = ECatalogues::findOrfail($req['id']);

        $result=  $ecatalogues->delete();
        // return redirect(route('admin.ecatalogues'))
        //     ->with('message', sprintf('ecatalogues "%s" deleted!', $ecatalogues->name));
        if ($result==1){
    ...
    echo json_encode($arr);
        ...
        // return response()->json(['message'=> sprintf('ECatalogues: "%s" deleted success !', $ecatalogues->name)]);
    }
    ...
    public function updateECatalogues(Request $request, $id)
    {
        ...
        if ($image && $image->isValid()) {
            $path = $image->storeAs('ecatalogues', basename($ecatalogues->image), 'public');
            $ecatalogues->image = $path;
        }
        ...
        $ecatalogues->save();
        return redirect(route('admin.ecatalogues'))->with('message', sprintf('ECatalogues: "%s" update success !', $ecatalogues->name));
    }
