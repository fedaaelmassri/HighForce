...
class ProductsController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($products->save()) {
            return redirect(route('admin.products'))->with([
                'message' => sprintf('The Brand: "%s" add success !', $products->name),
                'alert-type' => 'success'
            ]);
        } else {
            return redirect()->back()->with([
                'message' => sprintf('The Brand: "%s" can not add success !', $products->name),
                'alert-type' => 'error'
            ])->withInput();
        }
    ...
    public function updateBrand(Request $request, $id)
    {
        ...
        if ($products->save()) {
            return redirect(route('admin.products'))->with([
                'message' => sprintf(' The Brand: "%s" edit success !', $products->name),
                'alert-type' => 'success'
            ]);
        } else {
            return redirect()->back()->with([
                'message' => sprintf(' The Brand : "%s" can not edit success !', $products->name),
                'alert-type' => 'error'
            ])->withInput();
        }
     }
